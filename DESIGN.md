...
### Border Radius System (Subtle Enhancement)
...
- **Default Radius:** `4px` (`0.25rem`) for most interactive controls.
- **Maximum Radius:** `8px` for large containers and overlays only.
...
#### Usage by UI Primitive
- **Buttons / Inputs / Selects / Tabs:** 4px (`radius-sm`)
- **Cards / Panels / Module Blocks:** 6px (`radius-md`)
- **Dialogs / Drawers / Popovers:** 8px (`radius-lg`)
- **Chips / Status Dots / Avatars:** `full` allowed only for semantic circular elements

#### Consistency Rules
- Use one radius step per component family; do not mix 4px and 8px inside the same card block.
- Prefer `6px` for page-level content blocks to create mild visual polish.
- Keep table rows at 0px radius unless row grouping/selection needs a container treatment.
- Avoid decorative rounding on structural lines; preserve the technical, engineered look.
...
### Buttons
- **Primary:** Background `primary-container` (#FF5722), text `on-primary`. 0.25rem (4px) corner radius. No shadow.
...
- **Radius:** Use **MD (0.375rem / 6px)** by default for section cards. Use **DEFAULT (0.25rem / 4px)** for compact inner cards. Avoid `xl` or `full` except for status chips.
